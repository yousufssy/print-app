...
function formatOrderDetail(order: any) {
  ...
  const specsFields = [
    ...
    { label: 'الحجم النهائي (طري/قاسي)', value: (order.final_size_tall || order.final_size_width) ? `${order.final_size_tall || '?'} × ${order.final_size_width || '?'}` : '—' },
    { label: 'الحجم النهائي 2', value: (order.final_size_tall2 || order.final_size_width2) ? `${order.final_size_tall2 || '?'} × ${order.final_size_width2 || '?'}` : '—' },
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 12, color: 'var(--steel)', marginBottom: 8 }}>⚙️ مواصفات التفصيلة</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '6px 12px', fontSize: 12 }}>
                      {specsFields.map((f, idx) => f.value && f.value !== '—' ? (
                        <React.Fragment key={idx}>
                          <span style={{ fontWeight: 600, color: 'var(--steel)' }}>{f.label}:</span>
                          <span>{f.value}</span>
                        </React.Fragment>
