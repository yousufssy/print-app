...
function AccordionCard({
  title, children, defaultOpen = true, isOpen, onToggle,
}: {
  ...
  const toggle = () => { if (isControlled) onToggle?.(); else setInternalOpen(!open); };

  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 12, marginBottom: 16, background: '#fff', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      <button type="button" onClick={toggle} style={{ width: '100%', padding: '14px 20px', background: open ? 'var(--steel)' : 'var(--bg)', color: open ? '#fff' : 'var(--ink)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', fontSize: 14, fontWeight: 700, fontFamily: 'Cairo, sans-serif', transition: 'background 0.2s', textAlign: 'right', direction: 'rtl' }}>
        <span>{title}</span>
        <span style={{ fontSize: 14, transition: 'transform 0.25s ease', transform: open ? 'rotate(180deg)' : 'rotate(0)', display: 'inline-flex', alignItems: 'center' }}>▼</span>
      </button>
      <div style={{ maxHeight: open ? '3000px' : '0', overflow: 'hidden', transition: 'max-height 0.35s ease-in-out, opacity 0.25s ease', opacity: open ? 1 : 0 }}>
...
type TableRow = Record<string, string> & {
  _rowId: string;   // معرّف محلي ثابت طوال عمر الصف
  _isNew?: 'true';  // موجود فقط للصفوف التي لم تُحفظ بعد
  ID?: string;      // ID الحقيقي من قاعدة البيانات (غائب للصفوف الجديدة)
};

// ══════════════════════════════════════════════════════
// InlineTable — فصل كامل بين "حفظ صف واحد" و"حذف صف"
// ══════════════════════════════════════════════════════
const InlineTable = React.memo(function InlineTable({
  ...
  rows: TableRow[];
  /** يُستدعى لحفظ صف واحد فقط: إما إنشاء أو تحديث */
  onSaveRow: (row: TableRow) => Promise<void>;
  /** يُستدعى لحذف صف واحد */
  onDeleteRow: (row: TableRow) => Promise<void>;
  /** وضع المسودة: لا يوجد حفظ فوري — نخبر الأب بالتغييرات */
  syncDraftRows?: boolean;
  onDraftChange?: (rows: TableRow[]) => void;
}) {
  const [localRows, setLocalRows] = useState<TableRow[]>([]);
  const [saving, setSaving] = useState<Record<string, boolean>>({});
  const [edited, setEdited] = useState<Record<string, boolean>>({});

  // مزامنة مع rows الخارجية (عند تغيير isEdit أو refetch)
  useEffect(() => {
    setLocalRows(
      rows.map((r) => ({ ...r, _rowId: r._rowId || r.ID || crypto.randomUUID() }))
    );
    setEdited({});
  }, [rows]);
  ...
  const notifyDraft = useCallback((nextRows: TableRow[]) => {
    if (!syncDraftRows) return;
    onDraftChange?.(nextRows);
  }, [onDraftChange, syncDraftRows]);

  const addRow = useCallback(() => {
    const empty = Object.fromEntries(cols.map((c) => [c.key, ''])) as Record<string, string>;
    setLocalRows((prev) => {
      const nextRows: TableRow[] = [
        ...prev,
        { ...empty, ID: '', _isNew: 'true', _rowId: crypto.randomUUID() },
      ];
      notifyDraft(nextRows);
      return nextRows;
    });
  }, [cols, notifyDraft]);
  ...
  const setCell = useCallback((rowId: string, key: string, value: string) => {
    ...
    setLocalRows((prev) => {
      const nextRows = prev.map((r) =>
        r._rowId === rowId ? { ...r, [key]: finalValue } : r
      );
      if (syncDraftRows) notifyDraft(nextRows);
      return nextRows;
    });
    setEdited((prev) => ({ ...prev, [rowId]: true }));
  }, [isNumericCol, cleanNumber, syncDraftRows, notifyDraft]);
  ...
  const saveRow = useCallback(async (rowId: string) => {
    ...
    try {
      await onSaveRow(row);
      // إزالة علامة _isNew وedited بعد الحفظ
      setLocalRows((prev) =>
        prev.map((r) => {
      ...
      setEdited((prev) => ({ ...prev, [rowId]: false }));
    } catch (error) {
      console.error('❌ Save row error:', error);
    } finally {
      setSaving((s) => ({ ...s, [rowId]: false }));
    }
  }, [cols, localRows, onSaveRow]);

  const delRow = useCallback(async (rowId: string) => {
    const row = localRows.find((r) => r._rowId === rowId);
    if (!row) return;
    ...
    if (row._isNew === 'true') {
      // مجرد إزالة محلية
      setLocalRows((prev) => {
        const nextRows = prev.filter((r) => r._rowId !== rowId);
        notifyDraft(nextRows);
        return nextRows;
      });
    } else {
      await onDeleteRow(row);
      // الأب سيُعيد جلب البيانات → useEffect سيحدّث localRows
    }
  }, [localRows, notifyDraft, onDeleteRow]);
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead>
          <tr style={{ background: 'var(--steel)', color: '#fff' }}>
            ...
            <th style={{ padding: '8px 10px', width: 100 }}></th>
          </tr>
        </thead>
        <tbody>
          {localRows.length === 0 && (
          ...
          {localRows.map((row) => {
            ...
            const isEdited = !!edited[rowId];
            return (
              <tr key={rowId} style={{ borderBottom: '1px solid var(--border)', background: isNew ? '#fffbe6' : isEdited ? '#fff3cd' : localRows.indexOf(row) % 2 === 0 ? '#fff' : '#fdf8f0' }}>
                {cols.map((c) => {
                  ...
                  return (
                    <td key={c.key} style={{ padding: '3px 5px' }}>
                      <input
                        ...
                        onChange={(e) => setCell(rowId, c.key, isNumber ? cleanNumber(e.target.value) : e.target.value)}
                    ...
                    <>
                      {(isNew || isEdited) && (
                        <button type="button" onClick={() => saveRow(rowId)}
                          style={{ background: isEdited ? '#ffc107' : '#27ae60', border: 'none', borderRadius: 4, cursor: 'pointer', color: '#fff', fontSize: 11, padding: '4px 8px', marginLeft: 4, fontWeight: 600 }}
                          title="حفظ">💾</button>
                      )}
                      <button type="button" onClick={() => delRow(rowId)}
...
const MFG_MAP: Record<string, string> = {
  'برنيش': 'varnich', 'تلميع بقعي': 'uv_Spot', 'تلميع كامل': 'uv',
  'سلفان لميع': 'seluvan_lum', 'سلفان مات': 'seluvan_mat', 'طُبعت؟': 'Printed',
};
const CUST_MAP: Record<string, string> = {
  'مع طبخة': 'tabkha', 'مع تطوية': 'Tay', 'تدعيم زكزاك': 'Tad3em',
  'حراري': 'harary', 'بلص': 'bals',
};
...
export default function OrderFormPage() {
  ...
  // ── تحويل بيانات API إلى TableRow ─────────────────────
  const materialsRows = useMemo<TableRow[]>(() =>
    cartonsData.map((c: any) => ({
      _rowId: String(c.ID1),          // ← ثابت = ID الحقيقي
      ID: String(c.ID1 ?? ''),
      Type1: c.Type1 ?? '',
  ...
  const problemsRows = useMemo<TableRow[]>(() =>
  ...
  const operationsRows = useMemo<TableRow[]>(() =>
    operationsData.map((op: any) => ({
      _rowId: String(op.ID1 ?? op.ID),
  ...
  // ── مسودات للطلبات الجديدة (قبل الحفظ) ──────────────
  const [pendingMaterials, setPendingMaterials] = useState<TableRow[]>([]);
  const [pendingProblems, setPendingProblems] = useState<TableRow[]>([]);
  const [pendingOps, setPendingOps] = useState<TableRow[]>([]);

  // ══════════════════════════════════════════════════════
  // معالجات حفظ/حذف صف واحد — MATERIALS
  // ══════════════════════════════════════════════════════
  const handleSaveMaterial = useCallback(async (row: TableRow) => {
    const { _rowId, _isNew, ID, ...fields } = row;
  ...
  }, [id, year, createCarton, updateCarton, refetchCartons]);

  const handleDeleteMaterial = useCallback(async (row: TableRow) => {
    if (row.ID) await deleteCarton.mutateAsync(Number(row.ID));
    await refetchCartons();
  }, [deleteCarton, refetchCartons]);

  // ══════════════════════════════════════════════════════
  // معالجات حفظ/حذف صف واحد — PROBLEMS
  // ══════════════════════════════════════════════════════
  const handleSaveProblem = useCallback(async (row: TableRow) => {
    const { _rowId, _isNew, ID, ...fields } = row;
  ...
  }, [id, year, createProblem, updateProblem, refetchProblems]);

  const handleDeleteProblem = useCallback(async (row: TableRow) => {
    if (row.ID) await deleteProblem.mutateAsync(Number(row.ID));
    await refetchProblems();
  }, [deleteProblem, refetchProblems]);

  // ══════════════════════════════════════════════════════
  // معالجات حفظ/حذف صف واحد — OPERATIONS
  // ══════════════════════════════════════════════════════
  const handleSaveOperation = useCallback(async (row: TableRow) => {
    const { _rowId, _isNew, ID, ...fields } = row;
  ...
  }, [id, year, createOperation, updateOperation, refetchOperations]);

  const handleDeleteOperation = useCallback(async (row: TableRow) => {
    if (row.ID) await deleteOperation.mutateAsync(Number(row.ID));
    await refetchOperations();
  }, [deleteOperation, refetchOperations]);

  // ── حالة الأقسام (مع localStorage) ──────────────────
  ...
  useEffect(() => { localStorage.setItem('orderFormSections', JSON.stringify(openSections)); }, [openSections]);
  ...
  useEffect(() => {
    ...
    setChecks({
      varnich: fromBit(existing.varnich), uv: fromBit(existing.uv),
      uv_Spot: fromBit(existing.uv_Spot), seluvan_lum: fromBit(existing.seluvan_lum),
      seluvan_mat: fromBit(existing.seluvan_mat), Tad3em: fromBit(existing.Tad3em),
      Tay: fromBit(existing.Tay), harary: fromBit(existing.harary),
  ...
  useEffect(() => {
    ...
    const initData = { Ser: String(lastSer + 1), ID: newId, Year: currentYear };
  ...
  const onSubmit = useCallback(async (data: Order) => {
    try {
      ...
      if (isEdit) {
        ...
        await Promise.all([
          ...pendingMaterials.map(({ _rowId, _isNew, ID, ...f }) =>
            createCarton.mutateAsync({ ...f, ID: newId, year: yr }).catch(console.error)),
          ...pendingProblems.map(({ _rowId, _isNew, ID, ...f }) =>
            createProblem.mutateAsync({ ...f, ID: newId, Year: yr }).catch(console.error)),
          ...pendingOps.map(({ _rowId, _isNew, ID, ...f }) =>
            createOperation.mutateAsync({ ...f, ID: newId, Year: yr }).catch(console.error)),
        ]);
      }
  ...
  const handleDuplicate = useCallback(() => {
    ...
    const excludeFields = ['ID', 'ID1', 'Ser', 'Year', 'date_come', 'Perioud', 'marji3', 'AttachmentsOrders'];
    ...
    navigate('/orders/new', {
      state: {
        duplicatedData: {
          ...dataToCopy, Ser: '', Year: String(new Date().getFullYear()),
          checks: { ...checks }, mfgChecks: { ...mfgChecks }, custChecks: { ...custChecks },
        },
      },
    });
...
html, body { direction: rtl !important; font-family: Arial, sans-serif; background: #fff; }
.page { width: 210mm; min-height: 297mm; margin: 0 auto; padding: 8mm; border: 1px solid #000; direction: rtl !important; }
@media print {
  html, body { width: 210mm; height: 297mm; margin: 0; }
  .page { width: 210mm; height: 297mm; margin: 0 !important; padding: 8mm !important; border: none !important; page-break-after: always; }
}
table { width: 100%; border-collapse: collapse; direction: rtl !important; table-layout: fixed; word-break: break-word; }
...
.wrapper { display: flex; gap: 6px; align-items: flex-start; width: 100%; margin: 12px auto; }
...
.container { width: 100%; margin: 12px auto 0; border: 1.5px solid #000; padding: 10px; }
...
.checkbox { width: 11px; height: 11px; border: 1px solid #000; margin-left: 6px; flex-shrink: 0; text-align: center; font-size: 9px; line-height: 11px; display: inline-block; }
...
<div class="wrapper">
  <div class="main-container">
    <div class="grid-table">
      ...
      <div class="grid-item data-row"></div><div class="grid-item data-row"></div>
      <div class="grid-item data-row"></div><div class="grid-item data-row"></div>
      <div class="grid-item data-row"></div><div class="grid-item data-row"></div>
      <div class="grid-item data-row"></div>
    </div>
    <div class="bottom-section">
      <div><div style="height:30px;border:0.5px solid #000;"></div><div style="height:30px;border:0.5px solid #000;background:#999;"></div></div>
      <div><div class="label-cell">اخراج زيادة طبع</div><div class="label-cell grid-header">المجموع المستهلك في الطبعة</div></div>
    ...
    <div class="footer">
      <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
        <div style="display:grid;grid-template-columns:80px 130px;gap:4px;font-size:11px;">
          <div><div class="check-box"></div> تلف</div><div><div class="check-box"></div> خطأ</div>
          <div><div class="check-box"></div> زيادة كمية الطبع</div><div><div class="check-box"></div> تم معالجة الفروقات</div>
        </div>
        <div style="flex-grow:1;display:flex;align-items:baseline;margin-right:15px;font-size:11px;">
          ...
          <div style="border-bottom:1px dotted #000;flex-grow:1;margin-right:5px;"></div>
        </div>
      </div>
  ...
  <div class="side-table">
    ...
    <div class="side-cell" style="height:75px;">الطبع<span>على ${fmt(d.print_on)}</span><span>على ${fmt(d.print_on2)}</span></div>
...
<div class="container">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;margin-bottom:8px;">
    <div style="text-align:center;font-weight:bold;font-size:13px;">التفصيل للمقطع</div>
    <div style="text-align:center;font-weight:bold;font-size:13px;">تاريخ القطع: ___ / ___ / ٢٠</div>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
    <div>
      <div class="sketch-box"></div>
      <div style="display:grid;grid-template-columns:1.2fr 1fr;gap:8px;margin-top:8px;">
        <div>
          <div style="font-size:11px;margin-bottom:5px;">آلة الطبع: <span style="border-bottom:1px dotted #000;display:inline-block;width:60%;height:12px;">${fmt(d.Machin_Print)}</span></div>
          <div style="font-size:11px;margin-bottom:5px;">آلة التقطيع: <span style="border-bottom:1px dotted #000;display:inline-block;width:60%;height:12px;">${fmt(d.Machin_Cut)}</span></div>
          <div style="font-size:11px;margin-bottom:5px;">رقم القالب: <span style="border-bottom:1px dotted #000;display:inline-block;width:60%;height:12px;">${fmt(d.MontagNum)}</span></div>
        </div>
        <div>
          <div class="option-item"><div class="checkbox">${chkd(mfgChecks['برنيش'])}</div> برنيش</div>
          <div class="option-item"><div class="checkbox">${chkd(custChecks['مع تطوية'])}</div> مع تطوية</div>
      ...
      <div style="display:flex;align-items:center;margin-top:10px;">
        <div style="font-weight:bold;font-size:11px;margin-left:8px;white-space:nowrap;">الأبعاد:</div>
        <table style="flex-grow:1;border-collapse:collapse;">
          <tr><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;width:33%;">الطول</td><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;width:33%;">العرض</td><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;width:34%;">الإرتفاع</td></tr>
          <tr style="height:20px;"><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;">${fmt(d.LongU)}</td><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;">${fmt(d.WedthU)}</td><td style="border:1px solid #000;text-align:center;padding:4px;font-size:11px;">${fmt(d.HightU)}</td></tr>
        </table>
      </div>
    </div>
    <div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1.2fr;gap:8px;margin-top:8px;">
        <div>
          ...
          <div class="option-item"><div class="checkbox">${chkd(mfgChecks['سلفان مات'])}</div> سلفان مات</div>
          <div class="option-item"><div class="checkbox">${chkd(custChecks['حراري'])}</div> حراري</div>
          <div class="option-item"><div class="checkbox">${chkd(custChecks['بلص'])}</div> بلص</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:center;">
          <div style="font-weight:bold;margin-bottom:4px;font-size:12px;">ملاحظات:</div>
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button type="button" onClick={() => setOpenSections({ basic: true, specs: true, printing: true, quality: true, delivery: true })}
          style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 12px', fontSize: 12, cursor: 'pointer', color: 'var(--ink)', fontFamily: 'Cairo, sans-serif' }}>
          📂 فتح الكل
        </button>
        <button type="button" onClick={() => setOpenSections({ basic: false, specs: false, printing: false, quality: false, delivery: false })}
          style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 12px', fontSize: 12, cursor: 'pointer', color: 'var(--ink)', fontFamily: 'Cairo, sans-serif' }}>
          📁 إغلاق الكل
        </button>
        <Btn variant="outline" type="button" onClick={handleDuplicate} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          📄 نسخ الطلب
        </Btn>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <AccordionCard title="🎨 مواصفات المطبوعة" isOpen={openSections.specs} onToggle={() => toggleSection('specs')}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8,1fr)', gap: 12 }}>
            ...
            <G label="أرقام الكود"><input className="fc" {...register('Code_M')} style={{ textAlign: 'right' }} /></G>
          </div>
          ...
          {/* ✅ InlineTable الجديد — يمرر onSaveRow / onDeleteRow بدلاً من onRowsChange */}
          {isEdit ? (
            <InlineTable
              key={`materials-edit-${id}`}
              cols={MATERIALS_COLS}
              rows={materialsRows}
              onSaveRow={handleSaveMaterial}
            ...
            <InlineTable
              key="materials-new"
              cols={MATERIALS_COLS}
              rows={pendingMaterials}
              onSaveRow={async () => {/* لا شيء — syncDraftRows يتكفل */}}
              onDeleteRow={async () => {/* لا شيء */}}
              syncDraftRows
              onDraftChange={setPendingMaterials}
            />
          )}
        </AccordionCard>
        ...
        <AccordionCard title="⚙️ مواصفات الطباعة والمونتاج" isOpen={openSections.printing} onToggle={() => toggleSection('printing')}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8,1fr)', gap: 12 }}>
            ...
            <G label="العدد المنتج">
              <input className="fc" type="number" {...register('grnd_qunt')} style={{ background: '#f0f9f0', borderColor: '#27ae60', textAlign: 'right' }} />
            </G>
          ...
          {isEdit ? (
            <InlineTable
              key={`operations-edit-${id}`}
              cols={OPERATIONS_COLS}
              rows={operationsRows}
              onSaveRow={handleSaveOperation}
              onDeleteRow={handleDeleteOperation}
            />
          ) : (
            <InlineTable
              key="operations-new"
        ...
        <AccordionCard title="🔍 مراقبة الجودة والمشاكل" isOpen={openSections.quality} onToggle={() => toggleSection('quality')}>
          ...
          {isEdit ? (
            <InlineTable
              key={`problems-edit-${id}`}
              cols={PROBLEMS_COLS}
              rows={problemsRows}
              onSaveRow={handleSaveProblem}
            ...
            <InlineTable
              ...
              onDeleteRow={async () => {}}
              syncDraftRows
              onDraftChange={setPendingProblems}
            />
          )}
        </AccordionCard>
        ...
        <AccordionCard title="🚚 التسليم والفوترة" isOpen={openSections.delivery} onToggle={() => toggleSection('delivery')}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>
                {vouchers.length === 0 && (
                  <tr><td colSpan={10} style={{ textAlign: 'center', color: 'var(--muted)', padding: 20 }}>✦ لا توجد إيصالات — اضغط ➕ لإضافة إيصال</td></tr>
                )}
                {vouchers.map((v, i) => (
                  <tr key={(v as any).ID1} style={{ borderBottom: '1px solid var(--border)', background: i % 2 === 0 ? '#fff' : '#fdf8f0' }}>
                    ...
                    <td style={{ padding: '8px', textAlign: 'center' }}>
                      <button type="button" onClick={() => confirm('حذف الإيصال؟') && deleteVoucher.mutate((v as any).ID1)}
